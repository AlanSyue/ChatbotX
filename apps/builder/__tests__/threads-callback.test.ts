...
vi.mock("@chatbotx.io/business", () => ({
  ...
  resolveWorkspaceFreezeReason: vi.fn(() => null),
...
describe("threads oauth callback regressions", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  test("rejects mismatched oauth state cookie before exchanging the code", async () => {
    mockGetThreadsOAuthStateCookie.mockResolvedValue({
      nonce: "wrong-nonce",
      workspaceId: "1",
    })
    ...
    expect(mockClearThreadsOAuthStateCookie).toHaveBeenCalledTimes(1)
    expect(mockExchangeThreadsCode).not.toHaveBeenCalled()
  })

  test("redirects duplicate Threads accounts back to settings", async () => {
    mockFindThreadsByUserId.mockResolvedValue({
      id: "integration-9",
    })
    ...
    expect(mockRedirect).toHaveBeenCalledWith(`${REFERER}&error=duplicated`)
  })

  test("redirects reconnects whose stored account no longer matches the oauth profile", async () => {
    mockGetThreadsOAuthStateCookie.mockResolvedValue({
      nonce: "nonce-1",
      workspaceId: "1",
    ...
    mockFindThreadsByIdForWorkspace.mockResolvedValue({
      id: "5",
      threadsUserId: "threads-user-other",
    })
    ...
    expect(mockRedirect).toHaveBeenCalledWith(
      `${REFERER}&reconnect=error&reason=accountNotFound`,
    )
  })
})
